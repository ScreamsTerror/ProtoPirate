...
typedef enum
{
    ...
    MercedesDecoderStepCheckPreambula,
    MercedesDecoderStepSaveDuration,
    MercedesDecoderStepCheckDuration,
} MercedesDecoderStep;
...
SubGhzProtocolStatus subghz_protocol_encoder_mercedes_deserialize(void *context, FlipperFormat *flipper_format);
...
const SubGhzProtocol mercedes_protocol_v0 = {
    .name = MERCEDES_PROTOCOL_V0_NAME,
...
{
    UNUSED(environment);
    SubGhzProtocolEncoderMercedes *instance = malloc(sizeof(SubGhzProtocolEncoderMercedes));
    instance->base.protocol = &mercedes_protocol_v0;
...
SubGhzProtocolStatus subghz_protocol_encoder_mercedes_deserialize(void *context, FlipperFormat *flipper_format)
{
    ...
    do
    {
        ...
        if (instance->generic.data_count_bit != 68)
        {
        ...
        // Parse hex key (68 bits = 8.5 bytes, but we'll store in 64-bit for now)
        uint64_t key = 0;
        ...
        {
            // Extract from key data (bits 20-51)
            instance->serial = (uint32_t)((key >> 20) & 0xFFFFFFFF);
        }
        ...
        {
            // Extract from key data (bits 12-19)
            instance->button = (key >> 12) & 0xFF;
        }
        ...
        {
            // Extract from key data (bits 52-67)
            instance->counter = (key >> 52) & 0xFFFF;
        }
...
static void subghz_protocol_encoder_mercedes_update_data(SubGhzProtocolEncoderMercedes *instance)
{
    // Reconstruct the 68-bit data with updated button and counter
    uint64_t data = 0;

    // Bits 0-11: Fixed header/sync (seems to be 0x888 based on decoder)
    data |= 0x888;

    // Bits 12-19: Button (8 bits)
    data |= ((uint64_t)(instance->button & 0xFF) << 12);

    // Bits 20-51: Serial (32 bits)
    data |= ((uint64_t)(instance->serial & 0xFFFFFFFF) << 20);

    // Bits 52-67: Counter (16 bits) - truncated for 64-bit storage
    data |= ((uint64_t)(instance->counter & 0xFFFF) << 52);
...
LevelDuration subghz_protocol_encoder_mercedes_yield(void *context)
{
    ...
    subghz_protocol_encoder_mercedes_update_data(instance);
    ...
    if (instance->preamble_count < 32)
    {
        if (instance->preamble_count % 2 == 0)
        {
            ...
            return level_duration_make(true, subghz_protocol_mercedes_const.te_short);
        }
        else
        {
            // LOW pulse
            instance->preamble_count++;
            return level_duration_make(false, subghz_protocol_mercedes_const.te_short);
        }
    }
    ...
    {
        instance->preamble_count++;
        return level_duration_make(true, subghz_protocol_mercedes_const.te_long);
    }
    ...
    if (instance->preamble_count == 33)
    {
        instance->preamble_count++;
        instance->data_bit_index = 0;
        return level_duration_make(false, subghz_protocol_mercedes_const.te_long);
    }

    // Send data bits (68 bits total)
    if (instance->data_bit_index < instance->generic.data_count_bit)
    {
        // Mercedes V0 encoding: Similar to Honda but different timing
        ...
        if (instance->send_high)
        {
            ...
            uint64_t bit_mask = 1ULL << (67 - instance->data_bit_index);
            uint8_t current_bit = (instance->generic.data & bit_mask) ? 1 : 0;

            instance->last_bit = current_bit;

            // HIGH duration based on bit value
            uint32_t duration = current_bit ? subghz_protocol_mercedes_const.te_long : subghz_protocol_mercedes_const.te_short;
        ...
        else
        {
            ...
            uint32_t duration = instance->last_bit ? subghz_protocol_mercedes_const.te_long : subghz_protocol_mercedes_const.te_short;
            ...
            if (instance->data_bit_index >= instance->generic.data_count_bit)
            {
                ...
                instance->is_running = false;
                return level_duration_make(false, duration + 20000); // 20ms gap
            }
...
{
    UNUSED(environment);
    SubGhzProtocolDecoderMercedes *instance = malloc(sizeof(SubGhzProtocolDecoderMercedes));
    instance->base.protocol = &mercedes_protocol_v0;
...
void subghz_protocol_decoder_mercedes_feed(void *context, bool level, uint32_t duration)
{
    ...
    case MercedesDecoderStepReset:
        if ((level) && (DURATION_DIFF(duration, subghz_protocol_mercedes_const.te_short) < subghz_protocol_mercedes_const.te_delta))
        {
            instance->decoder.parser_step = MercedesDecoderStepCheckPreambula;
            instance->decoder.te_last = duration;
            instance->header_count = 0;
        }
        break;
    case MercedesDecoderStepCheckPreambula:
        if (level)
        {
            if ((DURATION_DIFF(duration, subghz_protocol_mercedes_const.te_short) < subghz_protocol_mercedes_const.te_delta) ||
                (DURATION_DIFF(duration, subghz_protocol_mercedes_const.te_long) < subghz_protocol_mercedes_const.te_delta))
            {
                instance->decoder.te_last = duration;
            }
            else
            ...
            (DURATION_DIFF(duration, subghz_protocol_mercedes_const.te_short) < subghz_protocol_mercedes_const.te_delta) &&
            (DURATION_DIFF(instance->decoder.te_last, subghz_protocol_mercedes_const.te_short) < subghz_protocol_mercedes_const.te_delta))
        {
            ...
            (DURATION_DIFF(duration, subghz_protocol_mercedes_const.te_long) < subghz_protocol_mercedes_const.te_delta) &&
            (DURATION_DIFF(instance->decoder.te_last, subghz_protocol_mercedes_const.te_long) < subghz_protocol_mercedes_const.te_delta))
        {
            if (instance->header_count > 14)
            {
                instance->decoder.parser_step = MercedesDecoderStepSaveDuration;
        ...
        break;
    case MercedesDecoderStepSaveDuration:
        if (level)
        {
            if (duration >=
                (subghz_protocol_mercedes_const.te_long + subghz_protocol_mercedes_const.te_delta * 2UL))
            {
                instance->decoder.parser_step = MercedesDecoderStepReset;
                if (instance->decoder.decode_count_bit ==
                    subghz_protocol_mercedes_const.min_count_bit_for_found)
                {
            ...
            {
                instance->decoder.te_last = duration;
                instance->decoder.parser_step = MercedesDecoderStepCheckDuration;
            }
        }
        else
        {
        ...
        break;
    case MercedesDecoderStepCheckDuration:
        if (!level)
        {
            if ((DURATION_DIFF(instance->decoder.te_last, subghz_protocol_mercedes_const.te_short) < subghz_protocol_mercedes_const.te_delta) &&
                (DURATION_DIFF(duration, subghz_protocol_mercedes_const.te_short) < subghz_protocol_mercedes_const.te_delta))
            {
...
static void subghz_protocol_mercedes_check_remote_controller(SubGhzBlockGeneric *instance)
{
    instance->serial = (uint32_t)((instance->data >> 20) & 0xFFFFFFFF);
    instance->btn = (instance->data >> 12) & 0xFF;
    instance->cnt = (instance->data >> 52) & 0xFFFF;
}
    ...
    SubGhzRadioPreset *preset)
{
    ...
    if (ret == SubGhzProtocolStatusOk)
    {
        ...
        subghz_protocol_mercedes_check_remote_controller(&instance->generic);
